    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        loginText.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                try {
                    // Navigate back to login activity with clear top to ensure proper stack
                    Intent intent = new Intent(RegisterActivity.this, LoginActivity.class);
                    intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
                    startActivity(intent);
                    finish();
                } catch (Exception e) {
                    Log.e("RegisterActivity", "Error navigating to login: " + e.getMessage(), e);
                    // Fallback to just finishing this activity
                    finish();
                }
            }
        });
                        ...
                        usersRef.child(userId).setValue(user)
                                .addOnCompleteListener(dbTask -> {
                                    if (dbTask.isSuccessful()) {
                                        ...
                                        // Navigate back to login screen without flags that might disrupt the activity stack
                                        Intent intent = new Intent(RegisterActivity.this, LoginActivity.class);
                                        intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
                                        startActivity(intent);
                                        // Just finish this activity, not the entire stack
                                        finish();
                                    } else {
